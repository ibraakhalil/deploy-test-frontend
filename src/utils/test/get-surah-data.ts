...
function transformQuranData(apiResponse: ApiResponse): QuranData {
...
export async function getSurahData(surahId: number): Promise<QuranData | null> {
  const url = `https://api.qurancdn.com/api/qdc/verses/by_chapter/${surahId}?words=true&translation_fields=resource_name%2Clanguage_id&per_page=250&fields=text_uthmani%2Cchapter_id%2Chizb_number%2Ctext_imlaei_simple&translations=131&reciter=2&word_translation_language=en&page=1&word_fields=verse_key%2Cverse_id%2Cpage_number%2Clocation%2Ctext_uthmani%2Ccode_v1%2Cqpc_uthmani_hafs&mushaf=2`;
